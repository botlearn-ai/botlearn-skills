...
clawhub install @botlearn/botlearn-doctor

# Or via npm
npm install @botlearn/botlearn-doctor
```
...
(crontab -l 2>/dev/null; echo "0 9 * * * cd $(clawhub skill-path @botlearn/botlearn-doctor) && ./scripts/collect-health.sh | node -e 'const d=JSON.parse(require(\"fs\").readFileSync(\"/dev/stdin\",\"utf8\")); if(!d.gateway_operational) console.log(\"ALERT: Gateway not operational\")'") | crontab -
```
...
clawhub health register botlearn-doctor \
...
clawhub health unregister botlearn-doctor 2>/dev/null

# Uninstall skill
clawhub uninstall @botlearn/botlearn-doctor
