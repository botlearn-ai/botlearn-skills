...
function getJourneyDay(openclawHome: string): number {
...
function buildDayContent(day: number): string {
  if (day === 0) {
    return ''; // Journey not started, don't inject anything
  }
...
const handler: HookHandler = async (event) => {
  ...
  // Don't inject if journey hasn't started
  if (day === 0) return;
